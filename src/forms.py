...
class UploadForm(FlaskForm):
    ...
    cpf = StringField(
        "Texto (CPF, Nome, etc.)", validators=[Optional(), Length(min=2, max=50)]
    )
    color = ColorField("Cor do Texto", default="#c0c0c0")
    font_size = IntegerField(
        "Tamanho da Fonte", default=12, validators=[NumberRange(min=6, max=72)]
    )
    ...
    watermark_image = FileField(
        ...
        validators=[
            Optional(),
            FileAllowed(["png", "jpg", "jpeg"], "Apenas imagens PNG ou JPG!"),
        ],
    )

    opacity = IntegerField(
        "Opacidade (0-100)", default=15, validators=[NumberRange(min=0, max=100)]
    )
    ...
    file = FileField(
        "Escolher arquivo PDF", validators=[DataRequired(), FileAllowed(["pdf"])]
    )
